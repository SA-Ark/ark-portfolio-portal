    ...
    <div className="grid gap-8 xl:grid-cols-[0.45fr_1fr]">
      <Card>
        ...
        <CardContent className="space-y-3">
          {projects.slice(0, 14).map((item) => <button key={item.id} onClick={() => { setProjectId(item.id); setExtra([]); }} className={`w-full rounded-2xl border p-4 text-left text-base transition ${projectId === item.id ? "border-blue-500 bg-blue-500/10 text-white" : "border-zinc-800 bg-zinc-950 text-zinc-300 hover:border-zinc-700"}`}>{item.name}<span className="mt-1 block text-sm text-zinc-500">{item.status} • {item.owner}</span></button>)}
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          <div className="mb-6 max-h-[640px] space-y-4 overflow-y-auto pr-2">
            {[...thread.map((message) => ({ id: message.id, author: message.author, role: message.role, body: message.body, date: message.sentAt })), ...extra.map((body, index) => ({ id: `extra-${index}`, author: "Maya Chen", role: "Manager" as const, body, date: new Date().toISOString() }))].map((message) => <div key={message.id} className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4"><div className="mb-2 flex flex-wrap items-center gap-2"><p className="text-base font-medium text-white">{message.author}</p><Badge variant="outline">{message.role}</Badge><span className="text-sm text-zinc-500">{formatDate(message.date)}</span></div><p className="text-base leading-7 text-zinc-300">{message.body}</p></div>)}
          </div>
