      ...
      <section className="grid gap-8 xl:grid-cols-[0.7fr_1.3fr]">
        <Card>
          ...
          <CardContent className="space-y-3">
            {folders.map((folder) => <div key={folder} className="flex items-center justify-between rounded-2xl border border-zinc-800 bg-zinc-950 p-4 text-base text-white"><span>{folder}</span><Badge variant="outline">{documents.filter((document) => document.folder === folder).length}</Badge></div>)}
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent className="space-y-4">
            {documents.slice(0, 20).map((document) => {
              ...
              return <div key={document.id} className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-base font-medium text-white">{document.name}</p><Badge>{document.version}</Badge></div><p className="mt-2 text-sm text-zinc-500">{client?.name} • {document.folder} • {document.owner} • {formatDate(document.updatedAt)}</p></div>;
            })}
          </CardContent>
        </Card>
      </section>
