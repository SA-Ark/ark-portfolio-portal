...
export default function InvoicesPage() {
  ...
  return (
    <>
      ...
      <MotionSection className="grid gap-5 md:grid-cols-3">
      ...
      <MotionSection className="grid gap-8 xl:grid-cols-[1.1fr_0.9fr]">
        <Card>
              ...
              <Table>
                ...
                <TableBody>
                  {invoices.slice(0, 28).map((invoice) => {
                    ...
                    return <TableRow key={invoice.id}><TableCell className="font-medium text-white">{invoice.id}<span className="block text-sm text-zinc-500">Due {formatDate(invoice.dueDate)}</span></TableCell><TableCell>{client?.name}</TableCell><TableCell><StatusBadge status={invoice.status} /></TableCell><TableCell>{formatCurrency(invoice.amount)}</TableCell><TableCell><Dialog title={`${invoice.id} details`} trigger={<Button variant="secondary" size="sm">View</Button>}><div className="space-y-6"><div className="rounded-2xl border border-white/[0.06] bg-white/[0.04] backdrop-blur-xl p-5"><p className="text-sm text-zinc-500">Bill to</p><p className="mt-1 text-xl font-semibold text-white">{client?.name}</p><p className="mt-2 text-base text-zinc-400">Issued {formatDate(invoice.issueDate)} • Due {formatDate(invoice.dueDate)}</p></div><div className="space-y-3">{invoice.lineItems.map((item) => <div key={item.label} className="flex items-center justify-between rounded-2xl bg-white/[0.06] p-4 text-base"><span>{item.label}</span><span>{formatCurrency(item.amount)}</span></div>)}</div><div className="flex items-center justify-between border-t border-white/[0.06] pt-4 text-xl font-semibold"><span>Total</span><span>{formatCurrency(invoice.amount)}</span></div></div></Dialog></TableCell></TableRow>;
                  })}
                </TableBody>
              </Table>
            </div>
