    ...
    <>
      ...
      <section className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          <Card key={label as string}><CardContent className="p-6"><Icon className="mb-4 h-6 w-6 text-blue-400" /><p className="text-sm text-zinc-500">{label as string}</p><p className="mt-2 text-base font-medium text-white">{value as string}</p></CardContent></Card>
        ))}
      </section>

      <section className="grid gap-8 xl:grid-cols-2">
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><BriefcaseBusiness className="h-6 w-6 text-blue-400" /> Project timeline</CardTitle><CardDescription>Current and historical delivery milestones.</CardDescription></CardHeader>
          <CardContent className="space-y-4">
            {(clientProjects.length ? clientProjects : []).map((project) => (
              <div key={project.id} className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4">
      ...
      <section className="grid gap-8 xl:grid-cols-3">
        <Card>
          ...
          <CardContent className="space-y-3">
            {clientDocuments.slice(0, 5).map((doc) => <div key={doc.id} className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4"><p className="text-base font-medium text-white">{doc.name}</p><p className="text-sm text-zinc-500">{doc.folder} • {doc.version} • {formatDate(doc.updatedAt)}</p></div>)}
          </CardContent>
        </Card>
        <Card className="xl:col-span-2">
          ...
          <CardContent className="space-y-4">
            {clientMessages.slice(0, 7).map((message) => <div key={message.id} className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4"><div className="mb-2 flex flex-wrap items-center gap-2"><p className="text-base font-medium text-white">{message.author}</p><Badge variant="outline">{message.role}</Badge><span className="text-sm text-zinc-500">{formatDate(message.sentAt)}</span></div><p className="text-base leading-7 text-zinc-300">{message.body}</p></div>)}
          </CardContent>
        </Card>
      </section>
